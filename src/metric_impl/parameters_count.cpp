...
size_t getIndent(const std::string_view line) { return line.find_first_not_of(' '); }
    ...
    auto param_lines =
        ...
        std::views::take_while([parameters_block_end](auto &&line) {
            ...
            return line_end.first < parameters_block_end.first ||
                   ((line_end.first == parameters_block_end.first) && line_end.second <= parameters_block_end.second);
        });
...
std::string CountParametersMetric::Name() const { return "CountParameters"; }
